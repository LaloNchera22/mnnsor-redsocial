...
export default function Feed() {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (currentAnonId !== "GUEST" && currentAnonId !== "UNKNOWN") {
  ...
  const handleGoogleAuth = async () => {
    ...
    if (!supabase) {
      ...
      let newAnonId = '';
      for (let i = 0; i < 6; i++) {
        newAnonId += characters.charAt(Math.floor(Math.random() * characters.length));
      }
    ...
    <main className="max-w-3xl mx-auto p-4 md:p-8 min-h-screen">
      <header className="flex flex-col md:flex-row justify-between items-baseline mb-12 border-b-4 border-black pb-4 relative">
        ...
        <div className="flex gap-4 mt-4 md:mt-0 items-center">
          ...
          />
          {anonId === "GUEST" || anonId === "UNKNOWN" ? (
            <button onClick={handleGoogleAuth} disabled={isProcessing} className="text-xs font-bold uppercase underline hover:text-gray-600 disabled:text-gray-400">
      ...
      <section>
        ...
        {posts.length === 0 ? (
          ...
          posts.map(post => (
            <Post key={post.id} post={post} onFlag={(id) => {
              if (anonId === "GUEST" || anonId === "UNKNOWN") {
                showToast("SIGN IN TO FLAG CONTENT.", "error");
              } else {
                handleFlag(id);
              }
            }} />
