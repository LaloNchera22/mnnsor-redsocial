           ...
           <div className="absolute top-full right-0 mt-2 w-72 bg-white border-2 border-black p-4 z-50 max-h-64 overflow-y-auto">
             ...
             {notifications.length === 0 ? (
               ...
               <ul className="space-y-4">
                 {notifications.map(n => (
                   <li key={n.id as string} className="font-mono text-xs border-l-2 border-black pl-2 flex justify-between items-start">
                     <div>
                       <span className="font-bold text-red-600 mr-2">[{n.type as string}]</span>
                       {n.content as string}
                     </div>
                     <button onClick={() => handleMarkAsRead(n.id as string)} className="text-[10px] uppercase underline ml-2 hover:text-gray-600">MARK READ</button>
                   </li>
                 ))}
               </ul>
             )}
